...
from final_model_activation_test import build_1d_conv_autoencoder
import random
...
with strategy.scope():
    ...
    model = build_1d_conv_autoencoder(input_shape, output_num)
#    model = tf.keras.models.load_model('model4_check_epoch_01.keras')
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath='model8_check_epoch_{epoch:02d}.keras',
    ...
    checkpoint_callback2 = tf.keras.callbacks.ModelCheckpoint(
        filepath='best_model8_check_MinMax_Conv3D.keras', 
        save_best_only=True, 
        monitor='val_loss', 
        mode='min',
        save_freq='epoch'
